function K = get_k_length(leg_length)
    ...
    Ip1=mp1*((L1+LM1)^2+0.048^2)/12.0;  %摆杆转动惯量（方块，两个摆杆）
    ...
    % % 将二阶导数替换为 f1, f2, f3
    % eqn10 = subs(subs(subs(subs(subs(subs(subs(subs(subs(
    % eqn1, 
    % diff(theta,t,2), f1),     % 替换摆杆角加速度为 f1
    % diff(x,t,2), f2),         % 替换驱动轮线加速度为 f2
    % diff(phi,t,2), f3),       % 替换机体角加速度为 f3
    % diff(theta,t), d_theta),  % 替换摆杆角速度为 d_theta
    % diff(x,t), d_x),          % 替换驱动轮速度为 d_x
    % diff(phi,t), d_phi),      % 替换机体角速度为 d_phi
    % theta, theta0),           % 替换摆杆角度为平衡点 theta0
    % x, x0),                   % 替换驱动轮位移为平衡点 x0
    % phi, phi0);               % 替换机体角度为平衡点 phi0
